...
r+=1
lbl(A,r,"  1-1동 gross margin (%) ★","자체 전극·ex-AMPC EBITDA 9% (메자닌 $1bn 적격)"); R['ba11_gm']=r
...
R['netdebt']=kv(r,"Net debt incl. DOE loan ($mm) ★",6000,USD,"EV→Equity"); r+=1
...
R['hc_mezz']=kv(r,"HoldCo · 인프라 크레딧/메자닌 ($mm) ★",800,USD,"공통 조달 → 양 공장 후순위 출자"); r+=1
R['hc_rate']=kv(r,"HoldCo · 크레딧 비용률 ★",0.09,PCT,"통합조달 → 단독보다 저렴"); r+=1
# --- SKBA SPV (DOE-clean) ---
A.cell(r,2,"  ▸ SKBA SPV (DOE-clean · 외부지분 ≤19.9%)").font=fnt(9,True,P1C); r+=1
R['ba_debt']=kv(r,"SKBA · project debt ($mm) ★",400,USD,"개조 capex 조달"); r+=1
R['ba_rate']=kv(r,"SKBA · project debt 이자율 ★",0.06,PCT,"DOE-clean → 저리"); r+=1
R['ba_hc']=kv(r,"SKBA · HoldCo 후순위 출자 ($mm) ★",300,USD,"HoldCo 우산 배분"); r+=1
...
R['pref_bal']=kv(r,"SKOT · 우선주/메자닌 ($mm) ★",1000,USD,"링펜싱 격리 조달"); r+=1
R['pref_rate']=kv(r,"SKOT · 메자닌 배당률 ★",0.07,PCT,"(B)키커 부여→10%에서 인하"); r+=1
...
R['ot_hc']=kv(r,"SKOT · HoldCo 후순위 출자 ($mm) ★",500,USD,"waiver 전제·DOE 비담보 형태"); r+=2
...
cs.cell(r,2,"※ 메자닌 구조: (A)만기 2032 단축 (B)배당 7%+키커 (C)AMPC창내 amortizing → DSCR 개선 · DOE 이자만 · 각 SPV non-recourse").font=fnt(8,False,"8C8473",it=True); r+=2
