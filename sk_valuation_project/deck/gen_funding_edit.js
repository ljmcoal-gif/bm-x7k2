...
// ════════ S2 · 현황 & 개조 ════════
{
  ...
  finTbl(2.06,'공장 1 — SKBA (조지아)',INK,'파우치 · 모회사',
    [['명판 / 가동률(run-rate)','18.0 GWh / 85%',GREEN],['기존 차입 (금융기관보증)','5조원 ≈ $3.7bn',RED],['run-rate 매출','~$2.3bn'],['개조 ex-AMPC EBITDA','13% (목표)'],['기저 EBITDA (pre-SOP) ★','$0.2bn (2026~)',GREEN],['순자산 NAV (BS) ★','~$2.0bn']]);
  finTbl(4.82,'공장 2 — SKOT (테네시)',RED,'BA 100% · SOP ′28 하반기',
    [['명판 / 가동률(run-rate)','29.7 GWh / 82%',GREEN],['기존 차입 (DOE 융자)','$4.0bn',RED],['  └ 상환','이자만~\'30 → 이후분할 ★',MUTE],['run-rate 매출','~$3.5bn'],['ex-AMPC EBITDA','13% (목표)'],['순자산 NAV (BS) ★','~$3.0bn']]);
  // 우: 개조 금액 (다크 박스)
  ...
  const eff=['DC센터 수요 확보 → 가동률 85% (BA)·82% (OT) run-rate','run-rate ex-AMPC EBITDA $748mm (BA 295 + OT 452, 13% 마진)'];
  let ey=4.06;
  ...
  const barx=ax+0.22, barw=aw-0.44, bary=ay2+0.44, barh=0.24, exF=2.7/6.0;
  ...
  s.addText('ex-AMPC $2.7bn',{x:barx,y:bary,w:barw*exF,h:barh,margin:0,align:'center',fontFace:MONO,fontSize:7.5,bold:true,color:PAPER,valign:'middle'});
  s.addText('AMPC PV $3.3bn (55%)',{x:barx+barw*exF,y:bary,w:barw*(1-exF),h:barh,margin:0,align:'center',fontFace:MONO,fontSize:8,bold:true,color:DARK,valign:'middle'});
  s.addText([{text:'DCF EV $6.0bn',options:{color:PAPER,bold:true}},{text:' 의 55%가 한시 AMPC → 멀티플엔 ex-AMPC만. WACC 10.5%·TGR 2%·Ke 13%',options:{color:WARM}}],
    {x:ax+0.22,y:ay2+0.74,w:aw-0.4,h:0.22,margin:0,fontFace:BODY,fontSize:8,valign:'middle'});
  // KPI 2x2 (Space Grotesk)
  const kp=[['총 명판','47.7','GWh',INK],['run-rate EBITDA','748','$mm',GREEN],['개조 소요','1.4','bn',RED],['DCF EV','6.0','bn',RED]];
...
// ════════ S4 · 지분 성립 조건 (조달 시나리오) ════════
{
  ...
  const asm=[['DC블록 ASP','$155/kWh'],['가동률 (run-rate) ★','85%/82% DC수요',GREEN],['SKBA 기저 EBITDA ★','$0.2bn pre-SOP',GREEN],['run-rate ex-AMPC','$748mm',GREEN],['WACC / 영구성장 ★','10.5% / 2.0%',INK],['Cost of equity (Ke)','13.0%',INK],['신규 조달','$1.42bn'],['순차입금 (BA/OT)','$6.4bn'],['peer EV/EBITDA','8~13x 보수 (실측 p.5)']];
  let ay=2.06;
  ...
  const body4=[head4,
    srow('BASE  (그랜트10/지분30)','10%','20%','$8.5bn','11.4x','11.4%',GREEN,MUTE,GREENSOFT),
    srow('① CoC 20% + 그랜트 20%','20%','20%','$7.8bn','10.5x','10.5%',GREEN,MUTE,GREENSOFT),
    srow('② CoC 해제 → 희석 30%','10%','30%','$7.8bn','10.5x','10.5%',GREEN,MUTE,GREENSOFT),
    srow('③ CoC 완화 → 희석 49%','10%','49%','$7.3bn','9.7x','9.7%',GREEN,MUTE,GREENSOFT),
    srow('최대결합 (그랜트20+희석49)','20%','49%','$7.0bn','9.3x','9.3%',GREEN,GREEN,GREENSOFT)];
  ...
  s.addText('필요EBITDA% = 딜을 peer 하단(13x)에 맞추는 데 필요한 ex-AMPC 마진 · 현 13% 마진이 전 시나리오(9.3~11.4%)를 상회 → 전부 성립',
    {x:colR,y:2.06+0.40+5*0.44+0.02,w:colRW,h:0.2,margin:0,fontFace:BODY,fontSize:7.5,italic:true,color:MUTE2,valign:'middle'});
  // peer 밴드 바
  eb(s,colR,4.9,colRW,'peer 밴드(보수 8~13x) 대비 — 전 시나리오 밴드 내');
  const bx=colR+0.05, bw=colRW-0.7, by=5.55, lo=8, hi=13;
  ...
  s.addShape(LINE,{x:px(9.3),y:by-0.24,w:px(11.4)-px(9.3),h:0,line:{color:GREEN,width:2}});
  s.addText('전 시나리오 밴드 내 (9.3~11.4x)',{x:px(9.3)-0.6,y:by-0.44,w:px(11.4)-px(9.3)+1.2,h:0.18,margin:0,align:'center',fontFace:MONO,fontSize:7,bold:true,color:GREEN,valign:'middle'});
  [[11.4,'BASE',GREEN,1],[10.5,'①·②','1E7A4D',0],[9.7,'③','1E7A4D',0],[9.3,'최대',GREEN,1]].forEach(m=>{
  ...
  [8,9,10,11,12,13].forEach(t=>s.addText(t+'x',{x:px(t)-0.3,y:by+0.28,w:0.6,h:0.18,margin:0,align:'center',fontFace:MONO,fontSize:7,color:MUTE2,valign:'middle'}));
  // 결론 콜아웃 (다크)
  s.addShape(RR,{x:MX,y:6.34,w:W-2*MX,h:1.28,rectRadius:0.06,fill:{color:DARK},line:{type:'none'}});
  s.addText('결론 — ex-AMPC EBITDA 13%면 전 시나리오가 peer 밴드에 든다',{x:MX+0.3,y:6.44,w:W-2*MX-0.6,h:0.3,margin:0,fontFace:HEAD,bold:true,fontSize:13,color:REDLT,valign:'middle'});
  s.addText([{text:'ex-AMPC EBITDA 마진을 9%→13%로 올리면 run-rate EBITDA가 ',options:{color:WARM}},{text:'$518→$748mm',options:{color:GREENLT,bold:true}},{text:'로 커져 역산 필요 멀티플이 ',options:{color:WARM}},{text:'BASE 11.4x → 최대결합 9.3x',options:{color:GREENLT,bold:true}},{text:'로 하락, ',options:{color:WARM}},{text:'전 시나리오가 peer 밴드(8~13x) 내',options:{color:GREENLT,bold:true}},{text:'(2029E 분모면 11.5~14.0x). DCF 내재 ',options:{color:WARM}},{text:'8.1x',options:{color:GREENLT,bold:true}},{text:'. AMPC 비중도 ',options:{color:WARM}},{text:'76→55%',options:{color:GREENLT,bold:true}},{text:'로 낮아져 사업 자생력↑ — 마진이 딜 성립의 핵심 레버. WACC10.5%·TGR2%·순차입 $6.4bn ★',options:{color:WARM}}],
...
// ════════ S5 · Peer 멀티플 (8~13x) ════════
{
  ...
  const btbl=[
    ...
    brow('run-rate 30~32','$748mm','8.1x','9.3~11.4x',GREENSOFT),
    brow('2029E forward','$607mm','9.9x','11.5~14.0x',PAPER),
  ];
  ...
  s.addText([{text:'× $748mm  → EV $9.7bn  ',options:{color:WARM}},{text:'지분 +$3.3bn\n',options:{color:GREENLT,bold:true}},{text:'× $607mm  → EV $7.9bn  ',options:{color:WARM}},{text:'지분 +$1.5bn',options:{color:GREENLT,bold:true}}],
  ...
  eb(s,MX,5.32,W-2*MX,'풋볼필드 — 내재 EV ($bn) · run-rate $748mm 기준 · 보수/정상화 peer vs 역산 필요');
  const fx=MX+0.15, fw=W-2*MX-1.0, fby=6.0, flo=5, fhi=12;
  const fp=v=>fx+fw*(v-flo)/(fhi-flo);
  s.addShape(LINE,{x:fx,y:fby,w:fw,h:0,line:{color:WARM,width:2.5}});
  // 보수 8~13x → 4.4~7.2 (sand)
  s.addShape(RECT,{x:fp(5.98),y:fby-0.10,w:fp(9.72)-fp(5.98),h:0.20,fill:{color:SAND},line:{color:WARM,width:0.75}});
  s.addText('보수 8~13x  $6.0~9.7bn',{x:fp(5.98),y:fby+0.13,w:fp(9.72)-fp(5.98),h:0.2,margin:0,align:'center',fontFace:MONO,fontSize:7.5,color:MUTE,valign:'middle'});
  // 정상화 13~15x → 7.2~8.3 (green)
  s.addShape(RECT,{x:fp(9.72),y:fby-0.10,w:fp(11.22)-fp(9.72),h:0.20,fill:{color:GREENSOFT},line:{color:GREEN,width:0.75}});
  s.addText('정상화 13~15x  $9.7~11.2bn',{x:fp(9.72)-0.3,y:fby-0.34,w:fp(11.22)-fp(9.72)+0.9,h:0.2,margin:0,align:'center',fontFace:MONO,fontSize:7.5,bold:true,color:GREEN,valign:'middle'});
  // 역산 필요 12.6~15.4x → 7.0~8.5 (red bracket 위)
  s.addShape(LINE,{x:fp(6.96),y:fby-0.30,w:fp(8.53)-fp(6.96),h:0,line:{color:RED,width:2.5}});
  s.addText('역산 필요 9.3~11.4x',{x:fp(6.96)-0.3,y:fby-0.52,w:fp(8.53)-fp(6.96)+0.9,h:0.2,margin:0,align:'center',fontFace:MONO,fontSize:7.5,bold:true,color:RED,valign:'middle'});
  // 순차입금 손익분기선 6.4
  ...
  // DCF EV 마커 4.84
  s.addShape(OVAL,{x:fp(6.03)-0.06,y:fby-0.06,w:0.12,h:0.12,fill:{color:GREEN},line:{type:'none'}});
  s.addText('DCF $6.0bn·8.1x',{x:fp(6.03)-0.85,y:fby-0.34,w:1.7,h:0.2,margin:0,align:'center',fontFace:MONO,fontSize:7.5,bold:true,color:GREEN,valign:'middle'});
  [5,6,7,8,9,10,11,12].forEach(t=>s.addText('$'+t+'bn',{x:fp(t)-0.3,y:fby+0.44,w:0.6,h:0.16,margin:0,align:'center',fontFace:MONO,fontSize:6.5,color:MUTE2,valign:'middle'}));
  // 결론 콜아웃
  s.addShape(RR,{x:MX,y:6.78,w:W-2*MX,h:0.9,rectRadius:0.06,fill:{color:SAND},line:{type:'none'}});
  s.addText([{text:'[핵심]  ',options:{bold:true,color:RED}},{text:'ex-AMPC EBITDA 마진 9%→13% 시 run-rate EBITDA ',options:{color:INK}},{text:'$518→$748mm',options:{color:GREEN,bold:true}},{text:'. 역산 필요 ',options:{color:INK}},{text:'9.3~11.4x로 전 시나리오가 peer 밴드(8~13x) 진입',options:{color:GREEN,bold:true}},{text:'(2029E 분모면 11.5~14.0x). DCF 내재 ',options:{color:INK}},{text:'8.1x',options:{color:GREEN,bold:true}},{text:'. AMPC 비중 ',options:{color:INK}},{text:'76→55%',options:{color:GREEN,bold:true}},{text:'로 사업 자생력↑ — 규모(GWh)보다 마진(%)이 밸류를 좌우. peer는 자체보조금 포함(우리 ex-AMPC, 보수적).',options:{color:INK}}],
